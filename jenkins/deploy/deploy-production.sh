...
sudo docker pull ditas/data-utility-evaluator:production
# Get the host IP
HOST_IP="$(ip route get 8.8.8.8 | awk '{print $NF; exit}')"
# Run the docker mapping the ports and passing the host IP via the environmental variable "DOCKER_HOST_IP"
sudo docker run -p 50002:8080 -e DOCKER_HOST_IP=$HOST_IP --restart unless-stopped -d --name data-utility-evaluator ditas/data-utility-evaluator:production
ENDSSH
